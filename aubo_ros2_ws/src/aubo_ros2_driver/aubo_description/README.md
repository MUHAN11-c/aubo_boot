...
AUBO 机械臂 **URDF / Xacro** 描述包，供 MoveIt2、`ros2_control` 与各 launch 加载。

## 与本包同级的其它目录

- **`world_map/`**：历史 **ROS 1 catkin** 场景描述（`package.xml` format 2），用于独立仿真/地图示例；**通常不加入当前 ROS 2 colcon 工作空间编译**。若需使用，请在兼容的 ROS1 工作空间中构建。

## URDF 目录结构（原 `urdf/README.md`）

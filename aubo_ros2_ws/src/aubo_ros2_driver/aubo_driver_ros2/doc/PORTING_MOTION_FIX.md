# aubo_driver_ros2 移植与运动卡顿/抖动修复说明

本文档说明从 ROS1 `aubo_driver` 移植到 ROS2 `aubo_driver_ros2` 过程中，为消除**连续卡顿**、**卡顿噪音**及**末端抖动**所做的问题定位、迭代修复与最终解决方案，便于后续维护与参考。
...
#### feedToRosMotionLoop()（500Hz 取点线程）

- **批量排空**：每次循环用 `while(setRobotJointsByMoveIt()) batch++` 将 `buf_queue_` 中所有可用点一次性转入 `ros_motion_queue_`，避免单点转移造成数据滞留
- **超时关闭 `start_move_`**：当 `ros_motion_queue_` 持续为空超过 500 次循环（~1 秒）才关闭 `start_move_`，防止短暂空队列导致误关
...
while(rclcpp::ok()) {
    ...
    if(0 == robot_mac_size_service_.robotServiceGetRobotDiagnosisInfo(pub_diag)) {
        ...
        if(current_macsz == 0)
            std::this_thread::sleep_for(std::chrono::milliseconds(10));  // RIB=0 额外等待
    }
    // 2. 按 RIB 空间送点
...
4. **RIB=0 额外 10ms 等待**：给控制器处理时间，与 ROS1 一致
...
| **feedToRosMotionLoop** | 500Hz | 批量从 `buf_queue_` 取点转入 `ros_motion_queue_`；超时管理 `start_move_` |
| **publishWaypointToRobot** | ~250Hz | 实时查询 RIB → 按缺额送点 → 4ms 间隔 |
...
| **Fix12** | **完全恢复 ROS1 架构：实时 RIB 查询 + buffer_size_=400 + 4ms 间隔 + RIB=0 额外 10ms + 局部 pub_diag** | **问题解决** |
...
4. **feeder 应批量排空而非单点转移**：`buf_queue_` 到 `ros_motion_queue_` 的转移如果每次只转一点（每 2ms 一点，500Hz），在上游 200Hz 发布下虽够用，但在 TCP 延迟尖峰后恢复时会跟不上。批量排空消除了这一瓶颈。

5. **`start_move_` 关闭要有足够容忍度**：短暂的队列为空不代表运动结束，1 秒超时能有效避免误关。

---

*文档版本：与当前 `aubo_driver_ros2` 及 `aubo_robot_simulator_ros2` 代码一致（Fix12 最终版）。若后续修改线程、缓冲或送点逻辑，请同步更新本文档。*

# aubo_moveit_config — # ROS 2 Control 与 MoveIt 集成
...
- **ROS 2发行版**: Humble Hawksbill (Ubuntu 22.04)
...
```bash
## 创建工作空间
mkdir -p ~/aubo_boot/aubo_ros2_ws/src
cd ~/aubo_boot/aubo_ros2_ws/src
...
```bash
cd ~/aubo_boot/aubo_ros2_ws/src/aubo_ros2_driver
...
```bash
cd ~/aubo_boot/aubo_ros2_ws/src/aubo_ros2_driver/aubo_moveit_config/config
```
...
1. 检查XACRO文件是否存在：
```bash
find ~/aubo_boot/aubo_ros2_ws -name "aubo_ros2.xacro"
```
...
- `/opt/ros/humble/share/moveit_resources_panda_moveit_config/config/panda_ros_controllers.yaml`
- `/opt/ros/humble/share/moveit_resources_panda_moveit_config/config/panda_controllers.yaml`

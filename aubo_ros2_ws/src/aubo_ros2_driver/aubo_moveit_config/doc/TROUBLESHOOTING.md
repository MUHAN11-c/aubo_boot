...
ls $(ros2 pkg prefix aubo_moveit_config)/share/aubo_moveit_config/config/aubo_e5.urdf.xacro

## 4. 测试XACRO解析
xacro $(ros2 pkg prefix aubo_moveit_config)/share/aubo_moveit_config/config/aubo_e5.urdf.xacro aubo_type:=e5 > /tmp/test_urdf.urdf
...
```bash
## 手动测试XACRO
xacro $(ros2 pkg prefix aubo_moveit_config)/share/aubo_moveit_config/config/aubo_e5.urdf.xacro aubo_type:=e5

## 检查文件是否存在
find ~/aubo_boot/aubo_ros2_ws -name "aubo_e5.urdf.xacro"
```
...
```bash
## 从URDF提取关节名称
xacro $(ros2 pkg prefix aubo_moveit_config)/share/aubo_moveit_config/config/aubo_e5.urdf.xacro aubo_type:=e5 | grep -oP 'name="\K[^"]*(?=".*type="revolute")'
...
```bash
## 检查URDF中的机器人名称
xacro $(ros2 pkg prefix aubo_description)/share/aubo_description/urdf/xacro/inc/aubo_e5.urdf.xacro aubo_type:=e5 | grep -i "robot name"

## 检查SRDF中的机器人名称
grep -i "robot name" config/aubo_e5.srdf
```
...
2. **添加缺失的链接（如ee_link）：**
在 `aubo_e5.urdf.xacro` 中添加末端执行器链接（参考主文档）
...
## 工作空间结构
tree -L 3 ~/aubo_boot/aubo_ros2_ws/src/aubo_ros2_driver/
```
...
> 返回: [aubo_moveit_config README](../README.md)

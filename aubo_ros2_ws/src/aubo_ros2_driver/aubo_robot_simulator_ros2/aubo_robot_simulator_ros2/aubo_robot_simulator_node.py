                ...
                if is_new_traj:
                    ...
                    for _step_idx in range(1, n_steps + 1):
                        if self.motion_ctrl.cancel_trajectory != 0:
                            break
                        t = min(_step_idx * update_duration_sec, T_blend)
                        pos, vel, acc = _quintic_interpolate(last_goal_point, current_goal_point, t, T_blend)
                        ...
                        self._publish_cmd(last_sec + t, "boundary_blend")
                        _wait = _step_idx * update_duration_sec - (_time.perf_counter() - _timing_wall_start)
                        if _wait > 0.0001:
                            _time.sleep(_wait)
                    did_boundary_blend = True
                    move_duration_sec = 0.0
                else:
                    # 段间速度连续：用上一段末的实际速度作为本段起点，减轻速度突变导致的抖动
                    with self.motion_ctrl.lock:

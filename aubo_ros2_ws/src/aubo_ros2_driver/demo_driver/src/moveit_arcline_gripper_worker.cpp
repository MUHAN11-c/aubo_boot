 ...
 * @brief 夹爪快换 Worker 节点：继承 MoveitArclineDemo，run() 实现夹爪 2 换下 + 夹爪 0 换上。
 */
...
MoveitArclineGripperWorker::MoveitArclineGripperWorker(const rclcpp::NodeOptions& options) : MoveitArclineDemo(options)
{
  run_gripper_swap_srv_ = create_service<demo_interface::srv::RunGripperSwap>(
      "run_gripper_swap", std::bind(&MoveitArclineGripperWorker::runGripperSwapCallback, this, std::placeholders::_1,
                                    std::placeholders::_2));
...
// -----------------------------------------------------------------------------
//  main：创建 Worker 节点、多线程执行、等待服务、执行夹爪快换
// -----------------------------------------------------------------------------

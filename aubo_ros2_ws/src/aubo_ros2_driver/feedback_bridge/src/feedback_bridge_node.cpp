...
public:
    ...
    : Node("feedback_bridge_node")
  {
    ...
    this->declare_parameter<std::string>("joint_states_input_topic", "joint_states");
...
private:
  ...
  {
    // 创建新的 joint_states 消息（可以在这里进行修改）
    ...
    RCLCPP_DEBUG(this->get_logger(), "Forwarded joint_states with %zu joints", 
                 msg->name.size());
  }

/**
 * 3D / 2D TF 工具层：
 * - `IvgRos3dTfClient`：仅委托 `ROSLIB.ROS2TFClient`（tf2_web_republisher），供 ros3d 使用。
 * - `ivgBuildTfPath` / `ivgFindRelativeTransform` 等：供 2D 投影等从本地 TF 边表算相对位姿（与 ros3d TF 源独立）。
 */
...
import * as ROSLIB from 'roslib';
	...
	/** 组合变换 T_a * T_b（先 b 后 a，与 TF 链乘法约定一致）。 */
	...
	/** 从 ``edges[child]={parent,transform}`` 沿父链向上累乘，返回路径与累积变换列表。 */
	...
	/** 求 source→target：在两帧到公共祖先路径上对齐后组合逆变换。 */
...
/** ros3d 用 TF 门面：仅使用 ROSLIB.ROS2TFClient（官方 tf2_web_republisher 链路）。 */

...
export function useRosService() {
  ...
  async function executeGrasp(objectId: string, useVisual: boolean) {
    const svcName = settings.rosName('svc-execute-single-grasp', '/execute_single_grasp')
    const svcType = settings.serviceType('svc-execute-single-grasp', 'ivg_interfaces/srv/ExecuteGraspPose')
    return call(svcName, svcType, {

...
const static std::vector<percipio_stream_index_pair> PERCIPIO_IMAGE_STREAMS = { DEPTH_STREAM, COLOR_STREAM,
                                                                                LEFT_IR_STREAM, RIGHT_IR_STREAM };
...
template <class T>
void PercipioCameraNode::setAndGetNodeParameter(T& param, const std::string& param_name, const T& default_value,
...
void PercipioCameraNode::setupDevices()
{
  ...
  if (!device_ptr->hasRightIR())
  {
    RCLCPP_WARN_STREAM(rclcpp::get_logger(LOG_HEAD_PERCIPIO_CAMERA_NODE),
                       "Right-IR image data stream is not supported!");
...
void PercipioCameraNode::setupPublishers()
{
  ...
  if (color_point_cloud_enable)
  {
    color_point_cloud_pub_ = node_->create_publisher<sensor_msgs::msg::PointCloud2>(
        ...
        rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(point_cloud_qos_profile), point_cloud_qos_profile));
  }
  ...
  for (const auto& stream_index : PERCIPIO_IMAGE_STREAMS)
  {
    ...
    camera_info_publishers_[stream_index] = node_->create_publisher<sensor_msgs::msg::CameraInfo>(
        topic, rclcpp::QoS(rclcpp::QoSInitialization::from_rmw(camera_info_qos_profile), camera_info_qos_profile));
  }

  device_event_publisher_ =
      node_->create_publisher<std_msgs::msg::String>("device_event", rclcpp::QoS(1).transient_local());
}
...
bool PercipioCameraNode::updateParameter(const std::string& param_name, const rclcpp::Parameter& parameter)
{
  try
  {
    ...
    if (param_name == "max_speckle_size")
    {
      ...
      {
        RCLCPP_WARN_STREAM(rclcpp::get_logger(LOG_HEAD_PERCIPIO_CAMERA_NODE), "max_speckle_size must be >= 0");
        return false;
      }
      device_ptr->depth_speckle_filter_init(depth_speckle_filter_enable, max_speckle_size, max_speckle_diff);
      RCLCPP_INFO_STREAM(rclcpp::get_logger(LOG_HEAD_PERCIPIO_CAMERA_NODE),
    ...
    if (param_name == "max_speckle_diff")
    {
      ...
      {
        RCLCPP_WARN_STREAM(rclcpp::get_logger(LOG_HEAD_PERCIPIO_CAMERA_NODE), "max_speckle_diff must be >= 0");
        return false;
      }
      device_ptr->depth_speckle_filter_init(depth_speckle_filter_enable, max_speckle_size, max_speckle_diff);
      RCLCPP_INFO_STREAM(rclcpp::get_logger(LOG_HEAD_PERCIPIO_CAMERA_NODE),
    ...
    if (param_name == "laser_power")
    {
      ...
      if (new_value < -1)
      {
        RCLCPP_WARN_STREAM(rclcpp::get_logger(LOG_HEAD_PERCIPIO_CAMERA_NODE), "laser_power must be >= -1");
        return false;
      }
      ...
      {
        device_ptr->set_laser_power(m_laser_power);
        RCLCPP_INFO_STREAM(rclcpp::get_logger(LOG_HEAD_PERCIPIO_CAMERA_NODE), "Updated laser_power: " << m_laser_power);
      }
      return true;
    }
    ...
    if (param_name == "tof_channel")
    {
      ...
      if (new_value >= 0)
      {
        ...
        device_ptr->set_tof_channel(m_tof_channel);
        RCLCPP_INFO_STREAM(rclcpp::get_logger(LOG_HEAD_PERCIPIO_CAMERA_NODE), "Updated tof_channel: " << m_tof_channel);
      }
      return true;
    }
    ...
    if (param_name == "gvsp_resend")
    {
      ...
      device_ptr->enable_gvsp_resend(m_gvsp_resend);
      RCLCPP_INFO_STREAM(rclcpp::get_logger(LOG_HEAD_PERCIPIO_CAMERA_NODE), "Updated gvsp_resend: " << m_gvsp_resend);
      return true;
    }
    ...
    if (param_name == "color_aec_roi")
    {
      ...
      if (!roi_str.empty())
      {
        ...
        if (cnt == 4)
        {
          ...
          {
            device_ptr->update_color_aec_roi(roi[0], roi[1], roi[2], roi[3]);
            RCLCPP_INFO_STREAM(rclcpp::get_logger(LOG_HEAD_PERCIPIO_CAMERA_NODE), "Updated color_aec_roi: " << roi_str);
          }
        }
        else
        {
    ...
    if (param_name == "exposure_time")
    {
      ...
      if (new_value <= 0.0f)
      {
        RCLCPP_WARN_STREAM(rclcpp::get_logger(LOG_HEAD_PERCIPIO_CAMERA_NODE), "exposure_time must be > 0");
        return false;
      }
      ...
      {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger(LOG_HEAD_PERCIPIO_CAMERA_NODE), "Failed to set exposure_time");
        return false;
      }
    }
    ...
    if (param_name == "gain")
    {
      ...
      if (new_value < 0.0f)
      {
        RCLCPP_WARN_STREAM(rclcpp::get_logger(LOG_HEAD_PERCIPIO_CAMERA_NODE), "gain must be >= 0");
        return false;
      }
      if (device_ptr->set_gain(new_value))
      {
        RCLCPP_INFO_STREAM(rclcpp::get_logger(LOG_HEAD_PERCIPIO_CAMERA_NODE), "Updated gain: " << new_value);
        return true;
      }
      else
      {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger(LOG_HEAD_PERCIPIO_CAMERA_NODE), "Failed to set gain");
        return false;
      }
    }
...
{
  bool has_subscriber =
      image_publishers_[COLOR_STREAM] && image_publishers_[COLOR_STREAM]->get_subscription_count() > 0;
  ...
  image_publishers_[COLOR_STREAM]->publish(*image_msg);
}

void PercipioCameraNode::publishLeftIRFrame(percipio_camera::VideoStream& stream)
{
  bool has_subscriber =
      image_publishers_[LEFT_IR_STREAM] && image_publishers_[LEFT_IR_STREAM]->get_subscription_count() > 0;
  SUBSCRIVER_CHECK(has_subscriber);
  ...
  image_publishers_[LEFT_IR_STREAM]->publish(*image_msg);
}

void PercipioCameraNode::publishRightIRFrame(percipio_camera::VideoStream& stream)
{
  bool has_subscriber =
      image_publishers_[RIGHT_IR_STREAM] && image_publishers_[RIGHT_IR_STREAM]->get_subscription_count() > 0;
  SUBSCRIVER_CHECK(has_subscriber);
  ...
  image_publishers_[RIGHT_IR_STREAM]->publish(*image_msg);
}

void PercipioCameraNode::publishDepthFrame(percipio_camera::VideoStream& stream)
{
  bool has_subscriber =
      image_publishers_[DEPTH_STREAM] && image_publishers_[DEPTH_STREAM]->get_subscription_count() > 0;
  SUBSCRIVER_CHECK(has_subscriber);
  ...
  {
    auto image_msg =
        cv_bridge::CvImage(std_msgs::msg::Header(), sensor_msgs::image_encodings::TYPE_16UC1, image).toImageMsg();
    ...
    image_publishers_[DEPTH_STREAM]->publish(*image_msg);
  }
  else if (image.type() == CV_16SC3)
  {
    auto image_msg =
        cv_bridge::CvImage(std_msgs::msg::Header(), sensor_msgs::image_encodings::TYPE_16SC3, image).toImageMsg();
    image_msg->header.stamp = HWTimeUsToROSTime(stream.getDepthStramTimestamp());
    image_msg->is_bigendian = false;
...
void PercipioCameraNode::publishColorPointCloud(percipio_camera::VideoStream& stream)
{
  ...
  point_cloud_msg->point_step = addPointField(*point_cloud_msg, format_str, 1, sensor_msgs::msg::PointField::FLOAT32,
...
void PercipioCameraNode::publishStaticTF(const rclcpp::Time& t, const tf2::Vector3& trans, const tf2::Quaternion& q,

# -*- coding: utf-8 -*-
""" 训练用损失：objectness、view、grasp（score/angle/width/tolerance）及总 loss。
    Author: chenxi-wang
"""
...
def get_loss(end_points):
    """ 汇总 Stage1 objectness、Stage1 view、Stage2 grasp 损失，总 loss = objectness + view + 0.2*grasp。 """
...
def compute_objectness_loss(end_points):
    """ Stage1 objectness 损失：在 fp2 的 seed 上取 objectness_label，与 objectness_score 做交叉熵。 """
...
def compute_view_loss(end_points):
    """ Stage1 view 损失：view_score 与 batch_grasp_view_label 在 objectness 为真的 seed 上做 MSE。 """
...
def compute_grasp_loss(end_points, use_template_in_training=True):
    """ Stage2 抓取损失：在 top 视角下取标签中得分最高的 angle，计算 score/angle_cls/width/tolerance 的 loss。 """
    ...
    grasp_loss = grasp_score_loss + grasp_angle_class_loss + grasp_width_loss + grasp_tolerance_loss

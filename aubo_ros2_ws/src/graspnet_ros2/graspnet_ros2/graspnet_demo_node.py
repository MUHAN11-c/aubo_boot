...
def _get_graspnet_baseline_root():
    ...
    # 源码布局：graspnet_ros2/graspnet_ros2/graspnet_demo_node.py -> 上一级再 graspnet-baseline
    ...
    if os.path.isdir(src_root) and os.path.isdir(os.path.join(src_root, 'models')):
        return src_root
    # install 布局：lib/.../graspnet_ros2/graspnet_demo_node.py，数据在 share/graspnet_ros2/graspnet-baseline
    try:
        ...
        pass
    return src_root  # 回退，后续 import 可能仍报错
...
class GraspNetDemoNode(Node):
    ...
    def __init__(self):
        ...
        # launch 可能传入字符串 'true'/'false'，兼容 bool 与 string
        def _to_bool(pv):
            s = (pv.string_value or '').strip().lower()
            if s in ('true', 'false'):
                return s == 'true'
            return bool(pv.bool_value)
        _uo = self.get_parameter('use_open3d').get_parameter_value()
        self.use_open3d = _to_bool(_uo)
        _ar = self.get_parameter('auto_run').get_parameter_value()
        self.auto_run = _to_bool(_ar)
    ...
    def _o3d_to_ros2_pointcloud(self, o3d_cloud):
        ...
        header.stamp = self.get_clock().now().to_msg()  # 当前时间，避免 TF 外推到过去导致障碍物未加入规划场景

...
def launch_setup_robot_and_tf(context):
    ...
    launch_moveit_py_controller = LaunchConfiguration('launch_moveit_py_controller').perform(context).lower() in ('1', 'true', 'yes', 'on')
    ...
    # moveit_py 需要在 pipeline 名命名空间（如 "ompl"）下读取规划参数；
    # 这里不重复写参数值，仅复用 move_group 已有配置并做最小映射。
    moveit_py_controller_manager = (
        moveit_controllers_yaml.get('moveit_simple_controller_manager', moveit_controllers_yaml)
        if isinstance(moveit_controllers_yaml, dict)
        else {}
    )
    moveit_py_adapter_params = [
        {'planning_pipelines': {'pipeline_names': ['ompl'], 'default_planning_pipeline': 'ompl'}},
        {'ompl': ompl_planning_pipeline_config['move_group']},
        {'moveit_simple_controller_manager': moveit_py_controller_manager},
        {'moveit_controller_manager': moveit_controllers.get('moveit_controller_manager')},
    ]
    moveit_py_params = move_group_params + moveit_py_adapter_params
    ...
    if launch_moveit_py_controller:
        nodes.append(Node(
            package=PKG_GRASPNET,
            executable='grasp_motion_controller_moveit2_node',
            name='grasp_motion_controller_moveit2',
            output='screen',
    ...
    if rviz_config and os.path.exists(rviz_config):
        ...
        rviz_params = [p for p in rviz_params_raw if p is not None]
        nodes.append(Node(
            package='rviz2',
            executable='rviz2',
            name='rviz2',
...
def _declare_launch_arguments(package_path, baseline_dir):
    ...
    return [
        ...
        DeclareLaunchArgument('launch_moveit_py_controller', default_value='false',
                              description='是否启动 grasp_motion_controller_moveit2_node（并注入 MoveIt 参数）'),

"""咖啡拉花 IO 控制节点：通过 /aubo_driver/set_io 控制 DO2/DO4，订阅 /aubo_driver/io_states 反馈 DI 状态。"""
...
class LatteIONode(Node):
    def __init__(self):
        super().__init__('latte_io')
        ...
        self.get_logger().info('咖啡拉花 IO 节点已就绪')
    ...
    def _call_set_io(self, io_index: int, value: bool) -> bool:
        ...
        if not self.set_io_client.service_is_ready():
            self.get_logger().error('/aubo_driver/set_io 不可用')
            return False

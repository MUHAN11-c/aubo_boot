...
C = {"orig":"#1a1a1a","promp":"#2ecc71","dmp":"#e74c3c","ens":"#3498db"}
TOP5 = [32, 25, 23, 6, 8]
...
class TabGeneralize(QWidget):
    def __init__(self, parent=None):
        ...
        bar = QHBoxLayout()
        self.btn_cv = QPushButton("运行 LOO 交叉验证 (40条)"); bar.addWidget(self.btn_cv)
        self.progress = QProgressBar(); self.progress.setVisible(False); bar.addWidget(self.progress)
        bar.addStretch(); layout.addLayout(bar)

    def draw(self, results):
        if results is None: return
        self.fig.clear(); res = results
        gs = self.fig.add_gridspec(2,3, hspace=0.4, wspace=0.35)
        ...
        ax.set_title('泛化误差箱线图'); ax.set_ylabel('RMSE(mm)'); ax.grid(alpha=0.3, axis='y')

        # 过拟合分析
        ax = self.fig.add_subplot(gs[0,2])
        cats=['ProMP\n训练','ProMP\n泛化','DMP\n训练','DMP\n泛化']
        vals=[1.5, res["promp"].mean(), 93, res["dmp"].mean()]
        clrs=[C["promp"],C["promp"],C["dmp"],C["dmp"]]
        ...
        ax.scatter(eps, res["promp"], c=C["promp"], s=30, alpha=0.7, label='ProMP')
        ax.scatter(eps, res["dmp"], c=C["dmp"], s=20, alpha=0.5, label='DMP')

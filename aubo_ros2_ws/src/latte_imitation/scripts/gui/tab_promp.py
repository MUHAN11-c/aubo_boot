...
import os; CARTESIAN_DIR = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "resource", "cartesian", "right")
TOP5 = [32, 25, 23, 6, 8]
...
class TabPromp(QWidget):
    def __init__(self, parent=None):
        ...
        bar = QHBoxLayout()
        bar.addWidget(QLabel("示教集:"))
        self.demo_combo = QComboBox(); self.demo_combo.addItems(["top5","all"]); bar.addWidget(self.demo_combo)
        self.btn_run = QPushButton("运行推理"); bar.addWidget(self.btn_run); bar.addStretch()
    ...
    def run(self):
        ...
        for ep in eps:
            ...
            if os.path.exists(path):
                cart=CartesianTrajectory.load(path); demos.append(extract_forming(cart.positions)[0])
        if not demos: return
        promp=ProMP3D(n_basis=20,sigma=0.03); promp.learn_multiple(demos)
        T=promp.promp_x._T_learned; cond=promp.generate(T=T)
        individuals=[]
        for demo in demos:
            p=ProMP3D(n_basis=20,sigma=0.03); p.learn(demo); individuals.append(p.generate(T=T))
        stack=np.stack(individuals,axis=0); mean_traj=np.mean(stack,axis=0); std_traj=np.std(stack,axis=0)
        self.data={"demos":demos,"promp":promp,"cond":cond,"mean":mean_traj,"std":std_traj,"n":len(demos)}
        return self.data

    def draw(self):
        if self.data is None: return
        d=self.data; self.fig.clear()
        gs=self.fig.add_gridspec(2,3,hspace=0.4,wspace=0.3)
        T=len(d["cond"]); idx=np.arange(T)
        for col,(dim,lbl) in enumerate([(0,'X'),(1,'Y'),(2,'Z')]):
            ...
            for demo in d["demos"]:
                Td=len(demo); ax.plot(np.linspace(0,T-1,Td), demo[:,dim], 'gray', lw=0.2, alpha=0.4)
            ax.set_title(f'{lbl}维度 ±2σ置信带'); ax.set_xlabel('帧'); ax.set_ylabel(f'{lbl}(m)'); ax.legend(fontsize=7); ax.grid(alpha=0.3)
        for col,(d1,d2,l1,l2) in enumerate([(0,1,'X','Y'),(0,2,'X','Z')]):
            ax=self.fig.add_subplot(gs[1,col])
            ax.plot(d["cond"][:,d1], d["cond"][:,d2], 'g-', lw=2.0, label='条件推理')
            ax.plot(d["mean"][:,d1], d["mean"][:,d2], 'b--', lw=1.5, label='均值')

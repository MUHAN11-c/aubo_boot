...
class TfPersistent:
    """持久 TF 查询器 — 单节点长生命周期, 避免反复创建/销毁 喵~"""
    def __init__(self, base_frame="base_link", ee_link="tool_tcp"):
        self.base = base_frame; self.ee = ee_link
        self._node = None; self._buffer = None; self._listener = None
        self._start()
...
class TabTF(QWidget):
    def __init__(self, parent=None):
        ...
        self.fig = Figure(figsize=(16,5.5), dpi=100)
    ...
    def start_tf(self):
        """启动持久 TF 节点 (首次调用时) 喵~"""
        if self._tf is None:
            self._tf = TfPersistent()

    def poll_tf(self):
        self.start_tf()
        pose = self._tf.lookup(timeout=0.3)
        if pose is not None:
            self.tf_pose = pose
            self.lbl_status.setText("已连接"); self.lbl_status.setStyleSheet("color:green;font-weight:bold")
            self.lbl_pose.setText(f"({pose.position.x:.3f},{pose.position.y:.3f},{pose.position.z:.3f})")
            self.btn_retarget.setEnabled(True)
        else:
            self.lbl_status.setText("等待TF..."); self.lbl_status.setStyleSheet("color:orange")
    ...
    def retarget(self, form_raw):
        ...
        cart = CartesianTrajectory(positions=form_raw.astype(np.float32),
            orientations=np.tile(quat.astype(np.float32),(Tf,1)),
            timestamps=np.arange(Tf,dtype=np.float32)*DT, dt=DT, episode_idx=-1, frame_id="base_link")
    ...
    def draw(self, orig_traj):
        ...
        ax.plot(orig_traj[:,0],orig_traj[:,1],orig_traj[:,2], 'gray', lw=1.0, alpha=0.5)
        ax.plot(rt[:,0],rt[:,1],rt[:,2], 'g-', lw=1.5)
        ...
        for pos,(d1,d2,l1,l2) in enumerate([(0,1,'X','Y'),(0,2,'X','Z')],1):
            ...
            ax_p.plot(orig_traj[:,d1],orig_traj[:,d2], 'gray', lw=0.8, alpha=0.5)
            ax_p.plot(rt[:,d1],rt[:,d2], 'g-', lw=1.2)
            if self.tf_pose:
                p=self.tf_pose.position; ax_p.scatter([p.x],[p.y if d2==1 else p.z], c='red', s=60, marker='*')
            ax_p.set_xlabel(l1); ax_p.set_ylabel(l2); ax_p.set_title(f'{l1}{l2}'); ax_p.grid(alpha=0.3); ax_p.set_aspect('equal')
        self.canvas.draw_idle()

    def closeEvent(self, event):
        if self._tf is not None: self._tf.shutdown()

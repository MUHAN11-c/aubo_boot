...
DT = 0.05
C = {"orig":"#1a1a1a","promp":"#2ecc71","dmp":"#e74c3c","ens":"#3498db","form":"#f39c12"}
...
class TabTrain(QWidget):
    ...
    def draw(self, data, show_orig=True, show_promp=True, show_ens=True, ensemble=None):
        ...
        self.fig.clear()
        d = data; gs = self.fig.add_gridspec(2,3, hspace=0.4, wspace=0.35, height_ratios=[1,1.1])
        fm, rp = d["form_raw"], d["recon_p"]

        # (0,0) 完整轨迹剖面
        ax = self.fig.add_subplot(gs[0,0])
        t = np.arange(d["Tf"])*DT; zs = savgol_filter(d["pos_full"][:,2],21,3)
        ax.plot(t, zs, 'gray', lw=1.0, alpha=0.5, label='Z')
        ax.plot(t, d["pos_full"][:,0], 'blue', lw=0.8, alpha=0.3, label='X')
        ax.axvspan(d["fs"]*DT, d["fe"]*DT, alpha=0.12, color=C["form"], label='成形段')
        ax.set_title(f'ep{d["ep"]} 完整轨迹'); ax.set_xlabel('秒'); ax.legend(fontsize=7); ax.grid(alpha=0.3)
        ...
        if show_ens and ensemble is not None:
            e=ensemble; ax3.plot(e[:,0],e[:,1],e[:,2], color=C["ens"], lw=1.2, alpha=0.7, ls=':', label='Ensemble')
        ax3.set_xlim(rx); ax3.set_ylim(ry); ax3.set_zlim(rz); ax3.set_title('3D对比'); ax3.legend(fontsize=6)

        # (0,2) RMSE条形图
        ax_r = self.fig.add_subplot(gs[0,2])
        meths=['ProMP','DMP']; rmses=[d["rmse_p"], d["rmse_d"]]; clrs=[C["promp"],C["dmp"]]
        ...
        ax_xy.set_xlabel('X'); ax_xy.set_ylabel('Y'); ax_xy.set_title('XY投影'); ax_xy.legend(fontsize=6); ax_xy.grid(alpha=0.3)
        ...
        xv = np.diff(fm[:,0])/DT; nv=len(xv)
        if nv>30:
            nperseg=min(64,nv//4); noverlap=min(48,nv//5)
            if nperseg>2:
                f_spec,t_spec,Sxx=spectrogram(xv, fs=1/DT, nperseg=nperseg, noverlap=noverlap)
                ax_s.pcolormesh(t_spec, f_spec, 10*np.log10(Sxx+1e-10), shading='gouraud', cmap='viridis', vmin=-40, vmax=10)
        ax_s.set_title('X速度时频图'); ax_s.set_xlabel('秒'); ax_s.set_ylabel('Hz'); ax_s.set_ylim(0.3,10)
        ...
        if show_promp:
            ...
            err=np.sqrt(np.sum((fm-rp)**2,axis=1))*1000
            ax_xz.text(0.02,0.98, f'Max误差={err.max():.1f}mm', transform=ax_xz.transAxes, fontsize=8, va='top')
        ax_xz.set_xlabel('X'); ax_xz.set_ylabel('Z'); ax_xz.set_title('XZ投影'); ax_xz.legend(fontsize=6); ax_xz.grid(alpha=0.3)

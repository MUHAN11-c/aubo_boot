...
HEART_DIR = os.path.join(PKG_DIR, "resource", "heart"); TOP5 = [32, 25, 23, 6, 8]
...
class TrainWorker(QThread):
    ...
    def __init__(self, form_raw, nb_promp=20):
        super().__init__(); self.form_raw = form_raw; self.nb_promp = nb_promp
    def run(self):
        from latte_imitation.promp_learner import ProMP3D
        from latte_imitation.dmp_learner import DMP3D
        Tf = len(self.form_raw)
        promp = ProMP3D(n_basis=self.nb_promp, sigma=0.03); promp.learn(self.form_raw)
        rp = promp.generate(T=Tf)
        rp_rmse = np.sqrt(np.mean(np.sum((self.form_raw-rp)**2, axis=1)))*1000
        dmp = DMP3D(n_basis=25, dt=0.01); dmp.learn(self.form_raw, tau=1.0)
        rd = dmp.generate(tau=1.0, T=Tf)
...
class CalibrateWorker(QThread):
    ...
    def __init__(self, episodes="all"):
        super().__init__(); self.episodes = episodes
    def run(self):
        from latte_imitation.trajectory import CartesianTrajectory
        self.progress.emit("加载轨迹...")
        all_f = []
        eps = range(40) if self.episodes == "all" else TOP5
        for ep in eps:
...
class OptimizationWorker(QThread):
    """后台贝叶斯优化 — ProMP 超参数搜索"""
    progress = pyqtSignal(int, float, dict); finished = pyqtSignal(dict)
    def __init__(self, mode="sim", n_trials=50, all_demos=None):
        super().__init__(); self.mode=mode; self.n_trials=n_trials; self.all_demos=all_demos or []
    def run(self):
        from scripts.optimize_heart_params import OptimizationRunner
        runner = OptimizationRunner(mode=self.mode, all_demos=self.all_demos)
        def cb(trial_num, score, params, best_score, best_params):
            self.progress.emit(trial_num, score, dict(params))
        runner.callback = cb; runner.run(n_trials=self.n_trials)
        self.finished.emit({"best_score":runner.best_score, "best_params":runner.best_params})

...
def _default_model_path():
    """Default model path: use ROS2 share dir if available, else path relative to this file."""
    try:
        from ament_index_python.packages import get_package_share_directory
        return str(Path(get_package_share_directory("vision")) / "models" / "best.pt")
    except Exception:
        pass
    # Fallback: relative to this source file (e.g. when run from source before install)
    pkg_dir = Path(__file__).resolve().parent
    return str(pkg_dir / "yolov11" / "models" / "best.pt")

...
bool ROS2Communication::initialize(std::shared_ptr<ConfigReader> config_reader, const std::string& template_root, const std::string& calib_file, bool debug) {
    ...
    // 创建服务
    // 姿态估计服务：接收图像和对象ID，返回估计的姿态（抓取、准备、放置等位置）
    ...
    // 模板标准化服务：对模板进行标准化处理，生成标准化的模板特征和掩膜
...
void ROS2Communication::handleEstimatePose(
    ...
    try {
        ...
        try {
            ...
            response->pose_image.push_back(vis_image_msg);
        } catch (cv_bridge::Exception& e) {
...
bool ROS2Communication::loadHandEyeCalibration(
    ...
    try {
        ...
        RCLCPP_ERROR(node_->get_logger(), "加载标定文件失败: %s, 错误: %s", 
                    calibration_path.c_str(), e.what());
        return false;
    }
}

// 加载姿态JSON文件：从JSON文件读取机器人姿态
...
// 寻找最佳掩码对齐：通过旋转和缩放找到最佳匹配
// @param query_mask 查询掩码
// @param template_mask 模板掩码
// @param best_angle 输出最佳角度（度）
// @param best_scale 输出最佳缩放比例
// @param best_iou 输出最佳IoU值
// @return 找到有效匹配返回true，否则返回false
...
// @param template_features 模板特征
// @param best_match 输出最佳匹配结果
// @return 找到有效匹配返回true，否则返回false

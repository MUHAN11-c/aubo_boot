...
class ROS2Communication:
    ...
    def _try_software_trigger_before_capture(self) -> bool:
        """调用 /software_trigger 触发相机出图。

        MultiThreadedExecutor 下使用 call_async + fut.result(timeout) 等待响应。
        ...
        if self._sw_trigger_client is None:
            self._sw_trigger_client = self.node.create_client(
    ...
    def _rotation_matrix_to_euler_rpy(self, R: np.ndarray) -> np.ndarray:
        """旋转矩阵转欧拉角 RPY（委托 math_utils）"""
        from .math_utils import rotation_matrix_to_euler_rpy
        return rotation_matrix_to_euler_rpy(R)

    def _quaternion_to_rotation_matrix(self, q: np.ndarray) -> np.ndarray:
        """四元数转旋转矩阵（委托 math_utils）"""

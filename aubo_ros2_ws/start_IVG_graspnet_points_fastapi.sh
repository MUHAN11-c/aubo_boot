#!/bin/bash
# 请使用 bash 运行（./start_IVG_graspnet_points_fastapi.sh 或 bash start_IVG_graspnet_points_fastapi.sh），勿用 sh
...
# IVG 完整启动脚本（GraspNet Points + FastAPI Web）
# 按顺序启动所有必要的节点和服务
...
echo -e "${GREEN}视觉姿态估计 Web 界面地址: ${WEB_URL}/${NC}"
echo -e "${GREEN}兼容旧版 UI 地址: ${WEB_URL}/legacy-ui/index.html${NC}"
echo ""

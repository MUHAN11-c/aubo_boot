...
struct Printer< ::nodelet::NodeletLoadRequest_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::nodelet::NodeletLoadRequest_<ContainerAllocator>& v)
  {
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "remap_source_args: ";
    if (v.remap_source_args.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.remap_source_args.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.remap_source_args[i]);
    }
    if (v.remap_source_args.empty() || true)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "remap_target_args: ";
    if (v.remap_target_args.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.remap_target_args.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.remap_target_args[i]);
    }
    if (v.remap_target_args.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.my_argv.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<std::basic_string<char, std::char_traits<char>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<char>>>::stream(s, true ? std::string() : indent + "    ", v.my_argv[i]);
    }
    if (v.my_argv.empty() || true)
      s << "]";
    if (true || !indent.empty())

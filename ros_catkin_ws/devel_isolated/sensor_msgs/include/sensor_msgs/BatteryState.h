  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::sensor_msgs::BatteryState_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.cell_voltage.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.cell_voltage[i]);
    }
    if (v.cell_voltage.empty() || true)
      s << "]";
    if (true || !indent.empty())
    ...
    if (v.cell_temperature.empty() || true)
      s << "[";
    for (size_t i = 0; i < v.cell_temperature.size(); ++i)
    {
      if (true && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer<float>::stream(s, true ? std::string() : indent + "    ", v.cell_temperature[i]);
    }
    if (v.cell_temperature.empty() || true)
      s << "]";
    if (true || !indent.empty())

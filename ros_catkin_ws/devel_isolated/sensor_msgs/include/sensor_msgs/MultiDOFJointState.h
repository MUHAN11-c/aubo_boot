  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::sensor_msgs::MultiDOFJointState_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.twist.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::Twist_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.twist[i]);
    }
    if (v.twist.empty() || false)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.wrench.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::Wrench_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.wrench[i]);
    }
    if (v.wrench.empty() || false)
      s << "]";
  }

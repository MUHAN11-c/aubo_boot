  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::sensor_msgs::MultiEchoLaserScan_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.ranges.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::sensor_msgs::LaserEcho_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.ranges[i]);
    }
    if (v.ranges.empty() || false)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.intensities.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::sensor_msgs::LaserEcho_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.intensities[i]);
    }
    if (v.intensities.empty() || false)
      s << "]";
  }

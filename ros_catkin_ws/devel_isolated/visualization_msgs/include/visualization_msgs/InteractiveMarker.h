...
struct Printer< ::visualization_msgs::InteractiveMarker_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::visualization_msgs::InteractiveMarker_<ContainerAllocator>& v)
  {
    ...
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "menu_entries: ";
    if (v.menu_entries.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.menu_entries.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::visualization_msgs::MenuEntry_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.menu_entries[i]);
    }
    if (v.menu_entries.empty() || false)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.controls.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::visualization_msgs::InteractiveMarkerControl_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.controls[i]);
    }
    if (v.controls.empty() || false)
      s << "]";
  }

  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::sensor_msgs::PointCloud_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.points.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::Point32_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.points[i]);
    }
    if (v.points.empty() || false)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.channels.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::sensor_msgs::ChannelFloat32_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.channels[i]);
    }
    if (v.channels.empty() || false)
      s << "]";
  }

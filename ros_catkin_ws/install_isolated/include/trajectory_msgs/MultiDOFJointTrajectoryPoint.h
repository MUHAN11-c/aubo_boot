  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::trajectory_msgs::MultiDOFJointTrajectoryPoint_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.velocities.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::Twist_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.velocities[i]);
    }
    if (v.velocities.empty() || false)
      s << "]";
    if (true || !indent.empty())
      s << std::endl;
    s << indent << "accelerations: ";
    if (v.accelerations.empty() || false)
      s << "[";
    for (size_t i = 0; i < v.accelerations.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::Twist_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.accelerations[i]);
    }
    if (v.accelerations.empty() || false)
      s << "]";
    if (true || !indent.empty())

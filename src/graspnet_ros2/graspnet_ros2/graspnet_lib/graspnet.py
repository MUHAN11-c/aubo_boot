...
Author: chenxi-wang（vendored 自 graspnet-baseline，本工作区裁剪为纯推理通路：
训练专用分支与 label_generation/loss 依赖未随附）
"""
...
from graspnet_ros2.graspnet_lib.backbone import Pointnet2Backbone
from graspnet_ros2.graspnet_lib.loss_utils import (
    GRASP_MAX_TOLERANCE,
    GRASP_MAX_WIDTH,
    batch_viewpoint_params_to_matrix,
)
from graspnet_ros2.graspnet_lib.modules import ApproachNet, CloudCrop, OperationNet, ToleranceNet
...
class GraspNet(nn.Module):
    """ 完整 GraspNet：Stage1 + Stage2（纯推理）。 """

    def __init__(self, input_feature_dim=0, num_view=300, num_angle=12, num_depth=4, cylinder_radius=0.05, hmin=-0.02, hmax_list=[0.01,0.02,0.03,0.04]):
        super().__init__()
        self.view_estimator = GraspNetStage1(input_feature_dim, num_view)
        self.grasp_generator = GraspNetStage2(num_angle, num_depth, cylinder_radius, hmin, hmax_list)
...
def pred_decode(end_points):
    """
    将 end_points 中的各头预测解码为 (B, M, 17) 的抓取数组列表，每样本 (M, 17) 对应
    grasp_core.GraspList 的输入格式：[score, width, height, depth, R(9), t(3), object_id]。

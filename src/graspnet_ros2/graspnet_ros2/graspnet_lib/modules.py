# -*- coding: utf-8 -*-
"""
GraspNet 各子模块：ApproachNet（视角估计）、CloudCrop（圆柱裁剪）、OperationNet（抓取参数）、ToleranceNet（容差）.
Author: chenxi-wang（vendored 自 graspnet-baseline）
"""
...
from graspnet_ros2.graspnet_lib.loss_utils import batch_viewpoint_params_to_matrix, generate_grasp_views
from graspnet_ros2.graspnet_lib.pointnet2_ops import CylinderQueryAndGroup


class ApproachNet(nn.Module):
    """ 根据 seed 点特征预测 objectness（二类）与 num_view 个视角的得分，取每个 seed 的 top 视角作为 approach。 """
    ...
    def forward(self, seed_xyz, seed_features, end_points):
        ...
        # 每个 seed 取得分最高的视角索引 (B, num_seed)
        top_view_scores, top_view_inds = torch.max(view_score, dim=2)
        # 扩展为 (B, num_seed, 1, 3) 以便在 template_views 的 dim=2 上 gather
...
class CloudCrop(nn.Module):
    """ 以每个 seed 为中心、按 vp_rot 旋转到局部系后，在圆柱内按不同深度 hmax 采点，MLP+maxpool 得到 vp_features。 """
...
class OperationNet(nn.Module):
    """ 抓取参数头：对每个 seed、每个 angle、每个 depth 预测 score、angle 类、width（3*num_angle 通道）。 """
...
class ToleranceNet(nn.Module):
    """ 容差头：对每个 seed、每个 angle、每个 depth 预测 tolerance（num_angle 通道）。 """

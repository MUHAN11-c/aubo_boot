...
职责:
  全链路坐标变换的**单份事实源**（统一原 peach_scene_perception/tf_utils.py 与
  peach_target_reconstruction/tf_utils.py 两份重复实现，重构阶段 A1）。
...
def transform_msg_to_matrix(transform) -> np.ndarray:
    ...
    peach_scene_perception.tf_utils._transform_msg_to_matrix 与
    peach_target_reconstruction.tf_utils.transform_msg_to_matrix（两者
...
def invert_transform(T: np.ndarray) -> np.ndarray:
    ...
    （实现沿用原 peach_target_reconstruction.tf_utils.invert_transform。）
...
def relative_motion(T_a: np.ndarray, T_b: np.ndarray) -> tuple:
    ...
    无语义收益，不用。
    （实现沿用原 peach_target_reconstruction.tf_utils.relative_motion。）
...
def gravity_camera_from_R(R_out_cam: np.ndarray) -> np.ndarray:
    ...
    （实现沿用原 peach_scene_perception.tf_utils._gravity_camera_from_R。）

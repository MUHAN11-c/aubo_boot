...
```
peach_executor/
  peach_executor/       # import peach_executor.*
    executor_node.py
    harvest_fsm.py
    batch.py
    lifecycle_manager.py     # peach_lifecycle_manager 节点
...
| `config/peach_executor.yaml` + `config/executor_parameters.yaml` | 运行 yaml 与 generate_parameter_library_py 参数库源；根键都是节点名 `peach_executor` |
| `config/observability.yaml` | 监控参数 |
| `launch/harvest_system.launch.py` | 整栈 include；能力包 `autostart:=false` |

读批次：先 `react()` 表，再 `_run_harvest` 里对 `Command.NAVIGATE` / `SURVEY` / `BEGIN_SCENE` / `WAIT_LOCK` / `SELECT` / `DISPATCH` / `EXECUTE_FULL` / `SETTLE` 的分支。

## 谁调谁

本节点是客户端：`BeginScene`、`SurveyScene`、`BuildTargetModel`、`ExecuteTarget`、`CheckReachability`；到位一步 `_cmd_navigate` 固定座直通 `NAV_OK`（`NavigateToWorksite` 预留，不发动作）。取消时 `_cancel_inflight` 级联所有在飞 handle。
...
显式 RunHarvest（须 Lifecycle Active，且栈就绪）
  → 到位直通 NAV_OK（NavigateToWorksite 预留，无导航动作）
  → SurveyScene（技能：拍照位姿 + 关节复核；失败 survey_failed，不 Begin）
  → BeginScene（感知：重启收齐窗、scene_epoch++；换场才清身份）
  → WAIT_LOCK（本世代 target_set_locked 或 survey_wait_s）
  → 无目标则再扫（不 Begin），连续 empty_survey_limit 次空集结算；SURVEY_ONLY 锁定后只扫不采
  → execution_enabled=false：WAIT_LOCK 后直接结算
  → 循环未入账目标
...
```bash
ros2 launch peach_executor peach_executor.launch.py
ros2 launch peach_executor observability.launch.py
...
参数要点：`execution_enabled` 默认 false；`execute_pregrasp_only` 默认 true（停预抓取等 ACK）；`survey_wait_s` 15；`survey_dwell_s` 2；`empty_survey_limit` 2；`persist_ledger` true。

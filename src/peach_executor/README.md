...
本节点是客户端：`BeginScene`、`SurveyScene`、`BuildTargetModel`、`ExecuteTarget`；到位一步 `_cmd_navigate` 固定座直通 `NAV_OK`（`NavigateToWorksite` 预留，不发动作）。取消时 `_cancel_inflight` 级联所有在飞 handle。
...
参数要点：`execution_enabled` 默认 false；`execute_pregrasp_only` 默认 true（停预抓取等 ACK）；`survey_wait_s` 8；`empty_survey_limit` 2；`persist_ledger` true。

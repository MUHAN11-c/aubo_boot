"""
抓取几何坐标变换 — 感知专属的 TF 残留层（基于 peach_perception.common.tf_utils 原语）.

职责:
  通用坐标变换原语（Transform 消息 ↔ 齐次矩阵、点/方向变换、旋转 ↔
  四元数值对象、重力反推）已统一迁移到 ``peach_perception.common.tf_utils``（重构阶段
  A3，消除与重建包的重复实现）；本模块只保留感知专属的两件事：
    1. ``_apply_T_to_grasp3d``：BagGraspReference3D 抓取几何整体的
       相机系 → 输出系变换（字段级规则为感知数据合约专属，不入纯核）；
    2. ``_rotation_to_quat``：peach_perception.common ``rotation_to_quat`` 返回的
       QuaternionValue → geometry_msgs/Quaternion 消息的薄包装
       （纯核不 import geometry_msgs，消息构造属编排层职责）。

坐标系/单位约定:
  与 peach_perception.common.tf_utils 一致：T 为 (4, 4)（输出系←输入系），点
...
def _rotation_to_quat(R: np.ndarray) -> Quaternion:
    ...
    peach_perception.common.tf_utils.rotation_to_quat），此处仅把 QuaternionValue

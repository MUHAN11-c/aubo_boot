"""
参数层 — ScenePerceptionParams：63 个参数的集中声明/装载/校验（frozen dataclass）.

对标 nav2 ParameterHandler 的 Python 版（设计文档
docs/superpowers/specs/2026-08-10-peach-layered-architecture.md §2.1）：

- ``ScenePerceptionParams.declare(node)``：集中 declare 全部参数与中文 descriptor
  （由节点原 ``_declare_params`` 整体搬入，键名/默认值/说明不变）；
- ``ScenePerceptionParams.from_node(node)``：集中读取/解析/校验 → frozen dataclass
...
``lock.impl`` 六键按名选择接口层实现（注册名见 scene_perception/impls.py
显式注册清单），节点构造期经 Registry.create 注入；A3 前由本层直接
构建 TargetRegistry 的做法已移除（匹配器/注册表改由节点装配）。

权威源哲学（AGENTS.md §7）：``config/scene_perception.yaml`` 是默认值权威源，
...
# 与 config/scene_perception.yaml 逐项对齐的默认值（test_params.py 双向同步强制）
DECLARE_DEFAULTS = {
    ...
    'gravity_mode': 'tf',
    # 2.14 装配：接口层实现的注册名（默认实现见 scene_perception/impls.py）
    'detector.impl': 'yolo',
    ...
    # （锚点反投影预识别，见 scene/segmentation_gate.py）；false 回退
    # 锁定后仍全量分割的旧行为
...
DESCRIPTIONS = {
    ...
    'calibration_version': '内外参版本标识（内参 color_camera_info.yaml；'
                           '外参 aubo_hand_eye_calibration/hand_eye/active.yaml）',
    ...
    'detector.impl': '检测器实现注册名（Detector 接口；默认 yolo='
                     'UltralyticsYolo，可选名见 scene_perception/impls.py）',
...
class ScenePerceptionParams:
    """
    ScenePerceptionNode 全部启动期参数（63 项 declare）装载后的不可变结构.
    ...
    @classmethod
    def from_node(cls, node) -> 'ScenePerceptionParams':
        """

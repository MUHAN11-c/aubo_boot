...
from .contracts import (
    BagGrasp2D, BagGraspReference3D, BagObservation, TOOL_GEOMETRY,
    ToolGeometry, compute_entry_start, compute_travel_range,
)
from .fitting import (
...
class RobustBagPosePipeline:
    ...
    def estimate(self, obs: BagObservation, target_id: str, bbox: tuple,
        ...
        base_2d.travel_line = [base_2d.grasp_px, self._project(entry + travel * axis, obs.camera_K)]
    ...
    def _failed(self, target_id, g2d, reason, source, **metrics):
        g2d.status = "REJECT"; g2d.diagnostic_flags = [reason]
    ...
    def _filter_depth_outliers(points, pixels):
        ...
        z = points[:, 2]; med = np.median(z); mad = np.median(np.abs(z - med))
        if mad < 1e-5:
            return points, pixels
    ...
    def _frame(axis, points):
        ...
        x /= np.linalg.norm(x)
        if x[0] < 0: x = -x
        y = np.cross(axis, x); y /= np.linalg.norm(y)
    ...
    def _project(point, K):
        if point is None or point[2] <= 1e-8: return None

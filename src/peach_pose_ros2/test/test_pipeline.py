...
class RobustBagPosePipelineTest(unittest.TestCase):
    def _observation(self, depth):
        return BagObservation(rgb=np.zeros((*depth.shape, 3), dtype=np.uint8), depth=depth,
                              camera_K=K, detections=[{"bbox": (500, 220, 780, 620), "class_id": 0, "conf": .9}])

    def test_valid_target_has_consistent_2d_3d_status(self):
        depth = np.zeros((720, 1280), dtype=np.uint16)
        depth[220:620, 500:780] = 900
        mask = np.zeros_like(depth, dtype=bool); mask[220:620, 500:780] = True
        result = RobustBagPosePipeline().estimate(self._observation(depth), "frame-1", (500, 220, 780, 620), mask, "test")
    ...
    def test_missing_depth_is_rejected(self):
        depth = np.zeros((720, 1280), dtype=np.uint16)
        result = RobustBagPosePipeline().estimate(self._observation(depth), "frame-1", (500, 220, 780, 620))
    ...
    def test_oversize_bag_is_never_accepted(self):
        ...
        mask = np.zeros_like(depth, dtype=bool); mask[160:660, 300:980] = True
        result = RobustBagPosePipeline().estimate(self._observation(depth), "frame-1", (300, 160, 980, 660), mask, "test")

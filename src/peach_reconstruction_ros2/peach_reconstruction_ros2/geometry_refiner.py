...
裸桃（'fruit'）→ 球拟合（半径窗 0.025~0.045 m）。拟合原语复用
peach_pose_ros2.peach_pose.fitting（fit_cylinder_robust/fit_sphere_robust——
官方库无带半径窗的圆柱/球 RANSAC，保留）；法线用 open3d 官方
estimate_normals（KDTree+协方差最小特征向量，与原手写 kNN+PCA 同算法，
A/B 方向一致到 |dot|=1.0；TSDF 云无序、无原始深度图可用，符号任意——
圆柱 RANSAC 只用 n₁×n₂ 定轴与射线交点定圆心，对法线符号不敏感）。
...
from peach_reconstruction_ros2.interfaces import GeometryRefiner
...
# 输出候选/诊断消息的 status 枚举（与 peach_pose_msgs 一致）
STATUS_ACCEPT = 0
STATUS_REOBSERVE = 1
STATUS_REJECT = 2
...
def refine_geometry(xyz: np.ndarray, target_kind: str = 'bag',
    ...
    xyz = np.asarray(xyz, dtype=np.float64)
    if xyz.size == 0:
        return _fail('empty_cloud', 0)
    xyz = xyz.reshape(-1, 3)
    n = xyz.shape[0]
    # 圆柱 RANSAC 最少 20 点（ransac_cylinder 下限），球线同样要求
    if n < 20:
        return _fail('insufficient_points', n)

    kind = 'sphere' if target_kind == 'fruit' else 'cylinder'
    if kind == 'cylinder':
    ...
    return {
        ...
        'radius': radius, 'diameter': 2.0 * radius, 'span_m': span,
        'rmse': rmse, 'inlier_ratio': inlier_ratio,
        'flags': flags,
    }


class RansacGeometryRefiner(GeometryRefiner):
    """
    interfaces.GeometryRefiner 的 RANSAC 实现薄壳（无状态，委托模块函数）.

    workhorse 本体是模块函数 refine_geometry（单测直接锚定）；本类只
    把调用形态对齐到 ABC（设计文档 §2.2，编排层按注册表实例化）。
    """
...
GEOMETRY_REFINERS = {'ransac': RansacGeometryRefiner}

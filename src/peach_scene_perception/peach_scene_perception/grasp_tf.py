...
def _apply_T_to_grasp3d(g3d, T: np.ndarray) -> None:
    ...
    points_centroid，走 peach_common_py transform_point）；方向只乘 R 并归一化
...
def _rotation_to_quat(R: np.ndarray) -> Quaternion:
    """
    3×3 旋转矩阵 → geometry_msgs/Quaternion（peach_common_py 值对象的消息包装）.

    数值路径与重构前完全一致：官方 quaternion_from_matrix（见
    peach_common_py.tf_utils.rotation_to_quat），此处仅把 QuaternionValue

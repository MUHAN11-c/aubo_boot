# Copyright 2026 wjz
"""
从 /api/state 各区段推导「当前这一颗」作业票（纯函数，无 ROS）.

给监控页一条从发现到抓取的进度线：档位关闭、观察失败、许可未过
不得再被画成后面环节已经勾上。几何一律 base_link，米。
"""
...
def build_harvest_job(snapshot: dict) -> dict:
    ...
    grasp = {
        ...
        'entry': _xyz(decision.get('entry')) if allowed else None,
        'axis': _xyz(decision.get('axis')) if allowed else None,
        'diameter_m': decision.get('diameter_m') if allowed else None,
        'rmse_m': decision.get('rmse_m') if allowed else None,
        'inlier_ratio': decision.get('inlier_ratio') if allowed else None,
    }
...
def _active_stage(batch: int, phase: int, message: str, skill: str,
    ...
    if batch == _BATCH_COMPLETED:
        ...
        return 'done', None
    if batch in (_BATCH_RECOVERY, _BATCH_INTERRUPTED):
        return 'done', 'done'
...
def _why(batch: int, phase: int, message: str, skill: str, grasp_on: bool,
    ...
    if not allowed:
        if recon_state in ('COLLECTING', 'REFINING'):
            return f'重建{recon_state}，尚未给出抓取许可'
        if reason:
            return f'抓取未许可：{reason}'
        if phase <= _PHASE_OBSERVING:

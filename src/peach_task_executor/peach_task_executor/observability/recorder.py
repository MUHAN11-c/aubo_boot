...
目录归属按「调度批次执行期」划分：batch_state 进入 DISCOVERY/RUNNING
...
可直接单测；Recorder 只被 observability 回调调用，所有盘写经队列 + 独立守护
...
目录结构（root 默认工作区 ``runs/``，与账本、重建 session 同一根）：
    ...
    <root>/run_<yyyyMMdd_HHmmss>/  # 一次批次一个文件夹
        ...
        manipulation.jsonl         # 技能节点 status 每次更新；hypothesis 带 topic
        job.jsonl                  # 当前果实作业票（过程线+几何，指纹去重）

# Copyright 2026 wjz
"""
HTTP 与 ROS 回调之间的线程安全最新值缓存（ObservabilityState）.
...
- 写侧（ROS 回调/采样线程）一律 copy-on-write：update/update_params 把
  ...
  存入的叶子；唯一原地改的是 task_executor.events 列表（追加+截头）。
...
- 因此 snapshot() 返回值是**只读**视图：消费方（http_server）拿到后只做
  序列化，禁止原地修改——改叶子会写穿到缓存。区段键与职责一致：
  perception / reconstruction / refined / manipulation / task_executor。
  另在锁外派生 `job`（当前果实作业票），不写入缓存。
"""

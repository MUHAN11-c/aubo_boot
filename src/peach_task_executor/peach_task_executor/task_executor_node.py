...
from peach_interfaces.action import (
    BuildTargetModel, ExecuteTarget, NavigateToWorksite, RunHarvest,
    SurveyScene)
    ...
    def _cmd_navigate(self, goal):
        ...
        if not arrived:
            ...
            if result is not None:
                code = str(getattr(result, 'failure_code', '') or '')
            self.get_logger().warning(
                f'NavigateToWorksite failed site={nav_goal.site_id or "-"} '
                f'code={code or "no_result"}')
        reaction = react(self._batch_state, event)
        self._apply(reaction, goal.request_id)
        return reaction

...
class DebugVisualizer:
    ...
    def create_depth_display(
        ...
        # 归一化到 uint8（直接使用原始深度图；简化版 Preprocessor 不再做 0 值插值）
        ...
        if np.any(valid_mask):
            ...
            depth_norm = np.zeros(processed_depth.shape, dtype=np.uint8)
            if max_val > min_val:
    ...
    def create_binary_display(
        ...
        if components and len(components) > 0:
            ...
            # 无连通域时回退为阈值二值图（与 Preprocessor._threshold 相同的无效值语义）
            depth = depth_image
            ...
            mask = (depth >= binary_threshold_min) & (depth <= binary_threshold_max) & ~invalid
            binary[mask] = 255

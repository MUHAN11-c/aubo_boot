...
class RemBGProcessor:
    ...
    def _ensure_session(self) -> None:
        ...
        # 模型目录指向包内 models/（运行文件自包含，避免落到 ~/.u2net）
        from .path_resolver import ensure_u2net_model_home
        ensure_u2net_model_home()
        ...
        try:
            # new_session 首次使用可能触发 u2net 模型下载（pooch），网络不可达
            # 会抛异常——与导入失败同等对待，process_roi 优雅回退 (None, None)

...
class ROS2Communication:
    ...
    def initialize(
        ...
        try:
            ...
            # 2D姿态估计服务：仅接收RGB图像，返回2D检测结果（中心坐标、旋转角度、置信度）
            # 服务名称: /estimate_pose_2d
            self.estimate_pose_2d_service = self.node.create_service(
                EstimatePose2D,
                'estimate_pose_2d',
                self._handle_estimate_pose_2d
            )
            ...
            # 参数已在 main.py 节点构造函数中统一声明，此处不再重复 declare_parameter

            # 持续订阅图像话题，缓存最新帧，供空图请求直接复用。
            # 使用 BEST_EFFORT QoS 兼容大多数相机驱动。
            ...
            self.depth_image_subscription = self.node.create_subscription(
                Image, depth_topic, self._depth_image_callback, camera_qos
            )
            self.color_image_subscription = self.node.create_subscription(
                Image, color_topic, self._color_image_callback, camera_qos
            )
            self.logger.info(f'已启用图像缓存订阅: {depth_topic}, {color_topic}')
            self.logger.info(f'  使用 QoS: BEST_EFFORT + KEEP_LAST(depth=1)')

            trigger_topic = str(
                self.node.get_parameter('soft_trigger_topic').value or '/camera/soft_trigger'
            )
            self._sw_trigger_pub = self.node.create_publisher(String, trigger_topic, 10)
            self._tf_buffer = Buffer()
            self._tf_listener = TransformListener(self._tf_buffer, self.node)
            self.logger.info(
                f'软触发话题: {trigger_topic}；T_B_C 优先查 TF '
                f'{self.node.get_parameter("base_frame").value} <- '
                f'{self.node.get_parameter("camera_optical_frame").value}'
            )
    ...
    def _capture_images_on_trigger(self, timeout=10.0):
        ...
        在 MultiThreadedExecutor 模式下，图像回调在后台线程运行，
        此方法用 threading.Event.wait() 阻塞直到两帧均到达或超时。

        Args:
            timeout: 超时时间（秒）

        Returns:
            (depth_image, color_image): 深度图和彩色图，超时则为 (None, None)
        """
        try:
            # 触发相机拍照
            ...
            while time.time() - start_time < timeout:
                with self.image_lock:
                    if (self.current_depth_image is not None and
                        ...
                        self.logger.info(
                            f'获取图像: 深度图={depth_copy.shape}, 彩色图={color_copy.shape}'
                        )
                        return depth_copy, color_copy
                time.sleep(0.05)

            self.logger.warning(f'获取图像超时 ({timeout}秒)')
            return None, None
    ...
    def _try_software_trigger_before_capture(self) -> bool:
        """发布 Percipio 软触发；无订阅者时不阻断，依赖自由出流缓存帧。"""
        if self._sw_trigger_pub is None:
            return False
        try:
            msg = String()
            msg.data = 'SoftTrigger'
            self._sw_trigger_pub.publish(msg)
            return True
        except Exception as e:
            self.logger.warning(f'软触发发布失败: {e}')
            return False
    ...
    def _load_hand_eye_calibration(self, calibration_path: str) -> bool:
        ...
        try:
            ...
            if 'hand_eye_calibration' in calib_data:
                ...
                # 本区 aubo_hand_eye_calibration/active.yaml：wrist_from_camera_optical（米）
                transforms = calib_data.get('transforms') if isinstance(calib_data, dict) else None
                if isinstance(transforms, dict):
                    entry = transforms.get('wrist_from_camera_optical') or {}
                    matrix = entry.get('matrix') if isinstance(entry, dict) else None
                    if matrix:
                        self.T_E_C = np.array(matrix, dtype=np.float64).reshape(4, 4)
                        loaded_anything = True
    ...
    def _get_current_T_B_E(self) -> Optional[np.ndarray]:
        """基座←末端（默认 base_link ← tcp）。"""
        base = str(self.node.get_parameter('base_frame').value or 'base_link')
        ee = str(self.node.get_parameter('ee_frame').value or 'tcp')
        return self._lookup_tf_matrix(base, ee)
    ...
    def _handle_estimate_pose_2d(
        self,
        request: EstimatePose2D.Request,
        response: EstimatePose2D.Response
    ) -> EstimatePose2D.Response:
        """
        处理2D姿态估计服务请求

        与 _handle_estimate_pose 完全相同流程，调用同一个 _process_single_feature。
        ...
        try:
            ...
            try:
                color_image = self.cv_bridge.imgmsg_to_cv2(request.image, desired_encoding='bgr8')
            except Exception as e:
                response.message = f'RGB图像解码失败: {e}'
                return response

            if color_image is None or color_image.size == 0:
                response.message = 'RGB图像为空'
                return response
            ...
            if depth_image is None:
                latest_depth, _ = self._capture_images_on_trigger(timeout=10.0)
                depth_image = latest_depth

            # 2. 验证图像完整性 —— 与 /estimate_pose 完全一致
            if not self._validate_images(depth_image, color_image, response):
                return response

            # 3. 深度图预处理和工件提取 —— 与 /estimate_pose 完全一致
            components, preprocessed_color = self._preprocess_images(depth_image, color_image)
            if not components:
                self.logger.warning('未检测到工件')
                response.message = '未检测到工件'
                return response

            # 4. 特征提取 —— 与 /estimate_pose 完全一致
            features = self._extract_features(components, preprocessed_color)
            if not features:
                self.logger.warning('未提取到有效特征')
                response.message = '未提取到有效特征'
                return response

            # 5. 加载模板库 —— 与 /estimate_pose 完全一致
            if not self._ensure_template_library_loaded(request.object_id, response):
                return response

            # 6. 姿态估计 —— 与 /estimate_pose 完全一致，调用同一个 _process_single_feature
            results = []
            for idx, feature in enumerate(features):
                result = self._process_single_feature(
                    idx, feature, features, components, color_image,
            ...
            # 7. 填充2D响应 —— 与 /estimate_pose 的 _fill_estimate_pose_response 对应
            response.success_num = len(results)
            for feature, result in results:
                response.center_x.append(float(feature.workpiece_center[0]))
                response.center_y.append(float(feature.workpiece_center[1]))
                response.rotation_angle.append(float(feature.standardized_angle_deg))
                response.confidence.append(float(result.confidence))
            ...
            if preprocessed_color is not None and features:
                try:
                    ...
                    response.vis_image = self.cv_bridge.cv2_to_imgmsg(vis_image, encoding='bgr8')
                except Exception as e:
                    self.logger.warning(f'2D可视化图像生成失败: {e}')

            response.message = f'2D姿态估计完成，检测到{len(results)}个目标'
            self.logger.info(response.message)

        except Exception as e:
            self.logger.error(f'2D姿态估计服务异常: {e}')
            response.message = f'2D姿态估计异常: {e}'
    ...
    def _process_single_feature(
        ...
        result = self._estimate_pose_for_feature(
            ...
            depth_image, distance, dtheta_rad, match_confidence=confidence
        )
        if result is None:
            return None
    ...
    def _load_camera_pose(
        ...
        """
        # 优先查本区 TF：base ← camera_color_optical_frame（extrinsics_publisher + 状态发布）
        T_B_C = None
        base = str(self.node.get_parameter('base_frame').value or 'base_link')
        cam_frame = str(
        ...
        if T_B_C is not None:
            pos = T_B_C[:3, 3]
            self.logger.info(
                f'从 TF 获取相机位姿 T_B_C ({base} <- {cam_frame}): '
            ...
            if T_B_E_current is not None and self.T_E_C is not None and not np.allclose(self.T_E_C, np.eye(4)):
                ...
                pos = T_B_C[:3, 3]
                self.logger.info(
                    f'从 TF 末端 + 手眼文件合成 T_B_C: 位置=({pos[0]:.4f}, {pos[1]:.4f}, {pos[2]:.4f})m'
                )
            else:
                T_B_C = np.eye(4)
                if T_B_E_current is None:
                    self.logger.warning('无法从 TF 获取相机/末端位姿，使用单位矩阵 T_B_C')
                elif self.T_E_C is None or np.allclose(self.T_E_C, np.eye(4)):
                    self.logger.warning('T_E_C 未设置或为单位矩阵，使用单位矩阵 T_B_C')
    ...
    def _get_images_from_request(
        ...
        # 1.3 如果请求中没有图像，使用持久订阅缓存的最新帧
        if depth_image is None or color_image is None:
            now_ts = time.time()
            max_age = float(self.node.get_parameter('cache_image_max_age_sec').value)
            with self.image_lock:
                if depth_image is None and self.current_depth_image is not None:
    ...
    def _handle_standardize_template(
        ...
        try:
            ...
            self.logger.info(f'模板目录下的pose_dirs: {[p.name for p in pose_dirs]}')
    ...
    def _standardize_single_pose_directory(
        ...
        self.logger.info(f'标准化图像, 标准化掩码, 裁剪参数字典成功')

        # 加载抓取姿态, 准备姿态, 相机位姿
        T_B_E_grasp, T_B_E_preparation, T_B_E_camera = self._load_pose_data(
            pose_id, pose_dir
        )
        self.logger.info(f'加载抓取姿态, 准备姿态, 相机位姿成功')

...
MOTION_UNAVAILABLE = "估姿 Web 不发运动/IO；请走 harvest 调试操作面或 MoveIt（graspnet_ros2）。"
...
class NativeWebService:
    ...
    def estimate_pose_2d(self, payload: dict[str, Any]) -> dict[str, Any]:
        node = self._require_node()
        ...
        if not color_image:
            raise HTTPException(status_code=400, detail="缺少输入图像（需要color_image）")
        if not object_id:
            raise HTTPException(status_code=400, detail="缺少工件ID")
        ...
        result, error = node.estimate_pose_2d(color_image, object_id)
        if error:
            raise HTTPException(status_code=500, detail=error)
        return result
    ...
    # ---- 运动/IO/抓取端点：本区无服务端，一律 501（路由保留以兼容旧 UI 按钮） ----

...
async function estimatePose2D() {
    if (_loopAutoGraspRunning) {
        addLogEntry('warning', '视觉循环抓取进行中，请先停止循环再手动执行2D估计');
        return;
    }

    if (!workflowState.currentColorImageBase64) {
        addLogEntry('warning', '请先采集图像（需要彩色图）');
        alert('请先采集图像（需要彩色图）');
        return;
    }
    ...
    addLogEntry('info', `开始2D姿态估计，工件ID: ${workpieceId}`);

    fetch(`${API_BASE_URL}/api/estimate_pose_2d`, {
        method: 'POST',
        ...
        .then(data => {
            if (data && data.success !== false) {
                const successNum = data.success_num || 0;
                addLogEntry('success', `2D姿态估计完成，检测到 ${successNum} 个目标${data.message ? '：' + data.message : ''}`);
                ...
                // 更新2D结果列表
                updateResultsList2D(data);

                for (let i = 0; i < successNum; i++) {
                    const cx = (data.center_x && data.center_x[i]) ? data.center_x[i] : 0;
                    const cy = (data.center_y && data.center_y[i]) ? data.center_y[i] : 0;
                    const angle = (data.rotation_angle && data.rotation_angle[i]) ? data.rotation_angle[i] : 0;
                    const conf = (data.confidence && data.confidence[i]) ? data.confidence[i] : 0;
                    addLogEntry('info',
                        `目标 ${i + 1}: 中心=(${cx.toFixed(1)}, ${cy.toFixed(1)})px, ` +
                        `旋转角=${angle.toFixed(2)}°, 置信度=${(conf * 100).toFixed(1)}%`
                    );
                }
            } else {
                addLogEntry('error', '2D姿态估计失败: ' + (data.message || data.error || '未知错误'));
            }
        })
        .catch(error => {
            addLogEntry('error', '2D姿态估计异常: ' + error.message);
            console.error('2D姿态估计错误详情:', error);
        });
}

function updateResultsList2D(data) {
    ...
    const successNum = data.success_num || 0;
    if (successNum === 0) {
        resultsList.innerHTML = '<div class="results-empty-msg">未检测到2D目标</div>';
        return;
    }
    ...
    for (let i = 0; i < successNum; i++) {
        const cx = data.center_x ? data.center_x[i] : 0;
        const cy = data.center_y ? data.center_y[i] : 0;
        const angle = data.rotation_angle ? data.rotation_angle[i] : 0;
        const conf = data.confidence ? data.confidence[i] : 0;
            ...
            <div style="padding: 8px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; padding-bottom: 6px; border-bottom: 1px solid #e2e8f0;">
                    ...
                    <div style="color: #64748b; font-size: 0.8em;">
                        置信度: <span style="color: #3b82f6; font-weight: 600;">${(conf * 100).toFixed(1)}%</span>
                    </div>
                </div>
                <div style="font-family: 'Courier New', monospace; font-size: 0.8em; line-height: 1.8; color: #475569;">
                    <div><strong>中心坐标:</strong> (${cx.toFixed(1)}, ${cy.toFixed(1)}) px</div>
                    <div><strong>旋转角度:</strong> ${angle.toFixed(2)}°</div>
                </div>
            </div>
        `;
...
function captureTemplateImage() {
    ...
    addLogEntry('info', `正在触发相机拍照，工件ID: ${workpieceId}，姿态ID: ${poseId}`);

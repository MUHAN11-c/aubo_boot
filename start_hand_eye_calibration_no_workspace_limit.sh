...
# 手眼标定完整启动脚本（无 MoveIt 工作空间限制）
# 基于 start_hand_eye_calibration.sh，去掉了 limit_workspace.py（边界墙）启动
# 按顺序启动所有必要的节点和服务
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}手眼标定系统启动脚本（无工作空间限制）${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
# （已移除）步骤4: MoveIt 工作空间限制（边界墙）
echo -e "${YELLOW}[4/10] 不启动 MoveIt 工作空间限制（边界墙），跳过 limit_workspace.py...${NC}"
echo -e "${GREEN}  ✓ 已跳过 MoveIt 工作空间限制，MoveIt 无边界墙限制${NC}"
sleep 1

# 步骤5: 启动机器人驱动服务
echo -e "${GREEN}[5/10] 启动机器人驱动服务...${NC}"
...
# 步骤8: 启动图像数据桥接节点
echo -e "${GREEN}[8/10] 启动图像数据桥接节点...${NC}"
...
echo -e "${GREEN}[10/10] 启动视觉姿态估计算法节点...${NC}"
...
echo ""
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}所有节点已启动完成！（无 MoveIt 工作空间限制）${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
echo -e "${GREEN}启动完成！手眼标定系统已就绪（无工作空间限制）。${NC}"

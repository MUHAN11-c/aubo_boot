...
struct GeometryGraph_
{
  ...
  _nodes_type nodes;
   ...
   typedef std::vector< ::graph_msgs::Edges_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::graph_msgs::Edges_<ContainerAllocator> >> _edges_type;
  _edges_type edges;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::graph_msgs::GeometryGraph_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.nodes.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::geometry_msgs::Point_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.nodes[i]);
    }
    if (v.nodes.empty() || false)
      s << "]";
    if (true || !indent.empty())
    ...
    for (size_t i = 0; i < v.edges.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::graph_msgs::Edges_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.edges[i]);
    }
    if (v.edges.empty() || false)
      s << "]";
  }
